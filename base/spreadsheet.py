...
class SpreadSheet(object):
    ...
    def get_worksheet_byname(self, spreadsheet_name, worksheet_name ):
        """ Get worksheet by name
        """
        ss = self.list_spreadsheets()
        for s in ss:
            if spreadsheet_name == s[0]:
                ws = self.list_worksheets(s[1])
                for w in ws:
                    if worksheet_name == w[0]:
                        return self.get_worksheet(s[1],w[1])
        return None
